...
def test_device_session_attributes(configure_database_mappers):
  ...
  index_columns = [[column.name for column in index.columns] for index in DeviceSession.__table__.indexes]

  for required_index in required_columns_indexes_tuples:
    assert required_index in index_columns, f"Required index {required_index} not found in device session table indexes. All available indexes: {index_columns}"
